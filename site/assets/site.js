document.addEventListener("DOMContentLoaded", () => {
  ...
  /* Scroll fade-up reveal — with a hard fallback so a missed observation
     never leaves a section permanently blank. */
  ...
  /* Discreet hover/focus captions on gallery tiles, sourced from alt text —
     no markup duplication required in every page. */
  document.querySelectorAll(".gallery-panel").forEach((panel) => {
    if (panel.querySelector(".gallery-panel__caption")) return;
    const img = panel.querySelector("img");
    const text = img ? img.alt : panel.getAttribute("aria-label");
    if (!text) return;
    const caption = document.createElement("span");
    caption.className = "gallery-panel__caption";
    caption.textContent = text;
    panel.appendChild(caption);
  });

  /* Contact form — preselect the audience dropdown from ?as=homeowner|investor|broker|designer
     so links from the dedicated partner pages land pre-tailored, not generic. */
  ...
  if (audienceSelect) {
    ...
    if (asParam && map[asParam]) {
      const option = Array.from(audienceSelect.options).find((o) => o.value === map[asParam]);
